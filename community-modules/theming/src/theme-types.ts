...
export type Part<T extends Param = never> = {
    readonly group: string;
    readonly id: string;
    readonly dependencies: readonly Part[];
    readonly defaults: Readonly<ParamDefaults<T>>;
     ...
     * of the same group
     */
    usePart<D extends Param>(part: Part<D>): Part<T | D>;

    /**
     * Provide new values for theme params. You may only provide values for
     * params provided by the part's dependencies.
     */
    overrideParams(defaults: Partial<Pick<ParamTypes, T | CoreParam>>): Part<T>;
     ...
     */
    addParams<A extends Param>(defaults: Pick<ParamTypes, A>): Part<T | A>;
};

export const createPart = (group: string, variant: string): Part => new ThemeUnit(group, variant);

export type Theme<T extends Param = never> = {
    readonly id: string;
    readonly dependencies: readonly Part[];
    readonly defaults: Readonly<ParamDefaults<T>>;
    readonly css: ReadonlyArray<CssFragment>;
     ...
     * of the same group
     */
    usePart<D extends Param>(part: Part<D>): Theme<T | D>;

    /**
     * Provide new values for theme params. You may only provide values for
     * params provided by the part's dependencies.
     */
    overrideParams(defaults: Partial<Pick<ParamTypes, T | CoreParam>>): Theme<T>;
...
export const createTheme = (name: string): Theme => new ThemeUnit('theme', name);

export type ParamDefaults<T extends string> = {
    [K in T]: K extends Param ? ParamTypes[K] : never;
};
